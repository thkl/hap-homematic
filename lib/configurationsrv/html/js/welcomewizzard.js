...
export class WelcomeWizzard extends Wizzard {
  ...
  createPage1 () {
    ...
    grid.setRenderer((row, room) => {
      ...
      let inputName = new Input('instancename_' + room.id, room.name, (e, input) => {
        self.instancesToCreate[room.id].create = input.value
      })
  ...
  createPage2 () {
    ...
    grid.setTitleLabels([self.__('Instance name'), self.__('Pin Code'), self.__('CCU Room')])
    grid.setColumns([
      {sz: {sm: 6, md: 3, lg: 3, xl: 3}},
      {sz: {sm: 6, md: 3, lg: 3, xl: 3}},
      {sz: {sm: 6, md: 5, lg: 5, xl: 5}}
    ...
    grid.setRenderer((row, bridge) => {
      ...
      return ([bridge.displayName, bridge.pincode, ccuRoomName])
    })
