...
export class NewDeviceWizzard extends Wizzard {
  constructor (application) {
    ...
    self.finishButton = new Button('success', self.__('Finish'), async (e, btn) => {
      if (self.newService.hapInstance === undefined) {
  ...
  async buildNewDeviceStep2 () {
    ...
    // Check if the Channel haz the default name
    // if yes we will use the device name

    self.newService.name = this.buildSuggestedName(self.newService.device, self.newService.channel) || channel.name
    ...
    let inputName = new Input('devicename', self.newService.name, (e, input) => {
