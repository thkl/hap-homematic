...
export class RebootUpdateDialog extends Wizzard {
  constructor (application) {
    ...
    this.debugControl.setLabel('Enable debug at launch')
    let proceedButton = new Button('danger', self.__('YOLO'), async (e, btn) => {
...
export class SetAuthenticationDialog extends Wizzard {
  constructor (application) {
    super(application)
    let self = this
    let useAuth = application.systemInfo.useAuth
    this.authControl = new CheckBox('useAuthentication', useAuth, (e, input) => {
      useAuth = input.checked
    })
    ...
    let proceedButton = new Button('info', self.__('Save'), async (e, btn) => {
      ...
      proceedButton.setActive(false)
      if (self.proceed) {
        self.proceed(useAuth)
      }
      self.dialog.close()
    }, true)

    this.dialog = new Dialog({
      dialogId: 'setAuthenticationDialog',
      buttons: [
        self.activitySpinner,
        self.dissmissButton,
        proceedButton
      ],
      title: self.__('Authentication'),
      dialogClass: 'modal-danger',
      scrollable: true,
      size: 'modal-md'
    })
