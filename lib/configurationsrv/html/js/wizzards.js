import {Dialog, Table, DatabaseTable, Button, Dropdown, Input, CheckBox, Spinner, Label} from './ui.js'
...
export class AbstractEditSettingsWizzard extends Wizzard {
  async buildDeviceSettings () {
    ...
    self.table.addRow([self.__('Homekit Device name'), inputName.render(), self.__('You may change the devicename as u like.')])
    self.table.addRow([self.__('Service'), oServiceList.render(), self.__('Select the service you want to use for this channel')])
    ...
    if (this.serviceSettings.template) {
      Object.keys(this.serviceSettings.template).map(settingsKey => {
        ...
        self.table.addRow([self.__(template.label || ''), (control) ? control.render() : '', self.__(template.hint || '')])
      })
    }

    self.table.addRow([self.__('HAP Instance'), hapList.render(), self.__('Select the HAP Instance to which you want to add this channel')])
