...
export class AbstractEditSettingsWizzard extends Wizzard {
  async buildDeviceSettings () {
    ...
    row.addCell({sz: {sm: 12, md: 2}}, self.__('Homekit Device name'))
    row.addCell({sz: {sm: 12, md: 5}}, inputName.render())
    row.addCell({sz: {sm: 12, md: 5}}, self.__('You may change the devicename as u like.'))

    row = self.grid.addRow('service')
    row.addCell({sz: {sm: 12, md: 2}}, self.__('Service'))
    row.addCell({sz: {sm: 12, md: 5}}, oServiceList.render())
    row.addCell({sz: {sm: 12, md: 5}}, self.__('Select the service you want to use for this channel'))
    ...
    if (this.serviceSettings.template) {
      Object.keys(this.serviceSettings.template).map(settingsKey => {
        ...
        row.addCell({sz: {sm: 12, md: 2}}, self.__(template.label || ''))
        row.addCell({sz: {sm: 12, md: 5}}, (control) ? control.render() : '')
        row.addCell({sz: {sm: 12, md: 5}}, self.__(template.hint || ''))
      })
    }

    row = self.grid.addRow('hapInstance')

    row.addCell({sz: {sm: 12, md: 2}}, self.__('HAP Instance'))
    row.addCell({sz: {sm: 12, md: 5}}, hapList.render())
    row.addCell({sz: {sm: 12, md: 5}}, self.__('Select the HAP Instance to which you want to add this channel'))
  }
}
