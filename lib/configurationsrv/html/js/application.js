...
import {NewDeviceWizzard, PublishDevicesWizzard, DeleteDeviceWizzard, DeleteHapInstanceWizzard, EditHapInstanceWizzard} from './wizzards.js'
...
export class Application {
  ...
  buildOverview () {
    ...
    bOv.empty()
    bOv.append(this.__('%s running HAP instances<br />', this.bridges.length))
    ...
    let sOv = $('#sysOverview')
    sOv.empty()
    sOv.append(this.__('%s cores %s <br /><br />', numCores, coreInfo))
  ...
  showInstances () {
    ...
    table.setColumns(['ID', self.__('Instance name'), self.__('PinCode'), self.__('Published devices'), '', ''])
  ...
  hook () {
    ...
    $('#reloadDevices').bind('click', (e) => {
      new PublishDevicesWizzard(this).run()
    })
