...
import {
  ...
  InvalidCredentialsDialog, SettingsDialog, SupportDialog
} from './wizzards.js'
...
export class Application {
  ...
  async buildOverview () {
    ...
    if ((self.systemInfo.forceRefresh === true) && (self.currentGrid)) {
      self.refreshDeleayed()
    }
  }
  ...
  refreshDeleayed() {
    switch (this.currentGrid.id) {
      case 'specialList':
        this.refreshSpecial()
        this.currentGrid.setDataset(this.specialList)
        this.currentGrid.updateBody()
        break;
    ...
    grid.setRenderer((row, item) => {
      let editButton = new Button('success', self.__('Edit'), (e, btn) => {
        ...
        wz.onExit = () => {
          self.refreshSpecial()
          container.empty()
          grid.setDataset(self.specialList)
  ...
  async refreshSpecial () {
    let tmp = await this.makeApiRequest({method: 'speciallist'})
    if (tmp) {
