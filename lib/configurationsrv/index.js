...
class ConfigurationService {
  ...
  saveVariable (query) {
    this.log.debug('[Config] save variable')
    ...
    if ((serial) && (newName) && (bridge)) {
      ...
      if (config.variables.indexOf(serial) === -1) {
        config.variables.push(serial)
      }
      if (config.mappings === undefined) {
        config.mappings = {}
      }
  ...
  removeVariable (serial, uuid) {
    if (serial) {
      this.log.debug('[Config] try to remove variable %s', serial)
      // remove the channel and the settings
      let configData = this.loadSettings()

      if ((configData) && (configData.variables)) {
        let index = configData.variables.indexOf(serial)
        if (index > -1) {
  ...
  async processApiCall (query, response) {
    if (query.method) {
      switch (query.method) {
        ...
        /** returns all known devices */
        case 'variablelist':
          this.sendJSON({variables: this.pluginVariables, trigger: this.pluginVariableTrigger}, response)
          break
        ...
        case 'removeVariable':
          this.sendJSON(this.removeVariable(query.serial, query.uuid), response)
          break
        ...
        case 'saveVariable':
          this.sendJSON(this.saveVariable(query), response)
          break
    ...
    if (message.topic) {
      ...
      switch (message.topic) {
        ...
        case 'variables':
          ...
          this.pluginVariableTrigger = message.trigger
          break
