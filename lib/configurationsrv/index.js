...
class ConfigurationService {
  ...
  updateSystem () {
    ...
    let backupFile = this.generateBackup()
  ...
  ccuCGICall (sid, method, parameters) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const req = http.request(options, (res) => {
        ...
        res.on('end', () => {
          // we will ignore the result cause the session was checked ealyer
          var result
          try {
  ...
  async getCCUFirewallConfiguration (sid) {
    let config = await this.ccuCGICall(sid, 'Firewall.getConfiguration')
    return config
  }
  ...
  generateBackup () {
    ...
    let backupCommand = 'tar -C ' + process.env.UIX_CONFIG_PATH + ' -czvf ' + backupFile + ' .'
    const childprocess = require('child_process')
    childprocess.execSync(backupCommand)
    return backupFile
  }
  ...
  async processApiCall (query, response) {
    ...
    if (!this.checkSid(query.sid, response)) {
      return
    }
    if (query.method) {
      ...
      switch (query.method) {
        ...
        case 'backup':
          ...
          response.writeHead(200, {
            'Content-Type': 'application/octet-stream',
            'Content-Disposition': 'attachment; filename=hap-homematic-backup.tar.gz',
  ...
  async sendObjects (sid) {
    // check the current firewall settings match the used ports
    let data = await this.getCCUFirewallConfiguration(sid)
    let fwConfig = data.result
    if (fwConfig) {
      this.bridges.map((bridge) => {
    ...
    this.sendMessageToSockets({message: 'serverdata',
      payload: {
