...
class ConfigurationService {
  ...
  async getSystemInfo () {
    ...
    if (fs.existsSync(packageFile)) {
      try {
        ...
        this.log.debug('[Config] version is %s', packageData.version)
        result.version = packageData.version
      } catch (e) {
        result.version = 'no version found'
      }
    } else {
      result.version = 'no version found'
    }
  ...
  updateSystem () {
    ...
    if (fs.existsSync(packageFile)) {
      try {
        ...
        return {'error': 'unable to get the update command'}
      }
    }
  }
  ...
  setAuthentication (query) {
    let switchOn = query.setOn
    let config = this.loadSettings()
    if (config === undefined) {
      config = {}
    }
    config.useCCCAuthentication = ((switchOn === true) || (switchOn === 'true'))
    this.saveSettings(config)
  }
  ...
  async processApiCall (query, response) {
    ...
    if (query.method) {
      ...
      switch (query.method) {
        ...
        case 'switchAuthentication':
          this.setAuthentication(query)
          this.sendJSON({'response': 'ok'}, response)
          this.restartSystem()
          break
