                ...
                if (typeof appl.settings.instance === 'object') {
                    appl.settings.instance.forEach(instID => {
                        ...
                        if (instance !== undefined) {
                            instNames.push({ id: instID, name: instance.displayName, remove: (instID !== appl.instanceID) });
                        }
                    })
                } else {
                    let instance = this.bridgeWithId(appl.instanceID)
                    if (instance !== undefined) {
                        instNames.push({ id: appl.instanceID, name: instance.displayName, remove: false });
                    }
                }
    ...
    saveDevice(data, configData, save) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (settings.instanceIDs !== undefined) {
                self.log.debug('[Config] settings up instances')
                instance = []
                ...
                if (fs.existsSync(clazzFile)) {
                    ...
                    Object.keys(settings).map((key) => {
                        if (Object.keys(oClazzSettings).indexOf(key) === -1) {
                            delete settings[key]
                            self.log.debug('[Config] removed %s which is not part of %s settings.', key, service)
                        }
                    })
                } else {
                    self.log.debug('[Config] clazzFile %s not found', clazzFile)
                }
