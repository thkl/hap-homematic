...
module.exports = class ConfigurationService {
    ...
    init() {
        ...
        if (((this.config.useTLS === true) || (forceTLS === true)) && (fs.existsSync(keyFile)) && (fs.existsSync(certFile))) {
        ...
        this.useAuth = this.config.useCCCAuthentication || false
    ...
    async getSystemInfo() {
        ...
        // check  my version
        this.log.debug('Check Registry Version')
        let strRegData = ''
        try {
            ...
            if (oReg) {
                ...
                this.log.debug('Found Registry Version %s', oReg['dist-tags'].latest)
            } else {
                this.log.debug('Unable to parse result %s', strRegData)
            }
        } catch (e) {
            this.log.debug('Unable to parse result %s', strRegData)
        }
