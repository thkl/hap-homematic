...
class HomeMaticCCU {
  ...
  async loadDatabases (configurationPath) {
    let self = this
    this.log.debug('[CCU] loading databases')
    this.devices = await this.loadObjectDatabase(path.join(configurationPath, 'devices.json'), 'devices', async () => {
      let result = await self.fetchAllDevices()
      return result
    })
    this.log.info('[CCU] device database loaded %s devices found', this.devices.length)

    this.variables = await this.loadObjectDatabase(path.join(configurationPath, 'variables.json'), 'variables', async () => {
      let result = await self.fetchVariables()
      return result
    })
    this.log.info('[CCU] variable database loaded %s variables found', this.variables.length)

    this.programs = await this.loadObjectDatabase(path.join(configurationPath, 'programs.json'), 'programs', async () => {
      let result = await self.fetchPrograms()
      return result
    })
    this.log.info('[CCU] program database loaded %s programs found', this.programs.length)

    this.rooms = await this.loadObjectDatabase(path.join(configurationPath, 'rooms.json'), 'rooms', async () => {
  ...
  loadObjectDatabase (databasePath, type, fetchFunction) {
    let self = this
  ...
  async updateDatabases (databasePath) {
    let self = this
    var result = await this.updateObjectDatabase(path.join(databasePath, 'devices.json'), async () => {
      let result = await self.fetchAllDevices()
      return result
    })
