...
class Server {
  ...
  async simulate (simPath, simData) {
    ...
    if (simData) {
      ...
      if ((simData.config) && (simData.devices)) {
        ...
        this.log.info('[Server] power up mockup bridge as we are in test mode')
        let channels = []
        // create a empty mapping if not set by the test
        this._configuration.channels.map(channelAddress => {
          let channel = {}

          // generate a dummy channel objectserivce used for
        ...
        this.log.info('[Server] test mode setup done')
      }
    }
  }
  ...
  loadInstance (instId, instanceData, ccuDevices, variables, programs, special, portInc) {
    ...
    var bridge
    if (this.isTestMode) {
      self.log.info('[Server] powering up test bridge : %s with userid %s and pinCode ', instanceName, username, pin)
      bridge = new BridgeMock(instanceName, uuid.generate(instanceName))
    } else {
      self.log.info('[Server] powering up bridge : %s with userid %s and pinCode ', instanceName, username, pin)
      bridge = new Bridge(instanceName, uuid.generate(instanceName))
    }
    ...
    bridge.instanceName = instanceName
  ...
  async publishServiceTable () {
    ...
    if (this.configUI) {
      ...
      this.log.warn('[Server] skipping service publishing to config server cause there is no config server (yet)')
    }
  }
          ...
          if (item.match(/HomeMatic.*Accessory.js/)) {
            ...
            if ((channelTypes) && (channelTypes.length > 0)) {
              channelTypes.map(channelType => {
                ...
                serviceConfig[channelType].push({serviceClazz: serviceName, settings: settings, priority: priority})
              })
            } else {
  ...
  _findServiceClass (channel) {
    ...
    if (sClass) {
      if ((typeof sClass === 'object') && (sClass.serviceClazz)) {
        ...
        if (instanceToAdd === forInstanceID) {
          ...
          if (sClass) {
            ...
            if (oInterface) {
              ...
              self.log.info('[Server] serivce used for %s is %s', channel.name, sClass)
