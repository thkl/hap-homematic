...
class Server {
  ...
  _findServiceClass (channel) {
    ...
    if (mappings) {
      ...
      if (sClass) {
        this.log.debug('[Server] service %s found thru address %s', sClass, channel.address)
        return sClass
      }
    }
    ...
    channelList.map(channel => {
      ...
      if (sClass) {
        ...
        if (oInterface) {
          ...
          let accessory = new Appliance(channel.name, oInterface.name, channel.address, channel.type, self)
          accessory.init()
