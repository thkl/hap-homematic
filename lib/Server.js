...
class Server {
  ...
  async connectCCU () {
    ...
    let lInstances = this._configuration.instances || {'0': {pin: '031-45-154', name: 'default'}}
    ...
    if (this._configuration.channels) {
      ...
      Promise.all(
        Object.keys(lInstances).map(instanceID => {
          ...
          let bridge = self.loadInstance(parseInt(instanceID), instanceData, channelsToMap)
  ...
  loadInstance (instId, instanceData, ccuDevices) {
    let self = this
    let pin = instanceData.pin || '031-45-154'
    let instanceName = 'HomeMatic_' + instanceData.name || 'HomeMatic_' + self.pad(instId, 2)
    let username = instanceData.user || this.getMacWithInstanceID(self.pad(instId, 2))
