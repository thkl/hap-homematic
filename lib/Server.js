...
class Server {
  constructor (log, configurationPath) {
    ...
    this._variableAccessories = [] // used to fetch all at the key event
    this._configuration = {}
  ...
  powerUpBridges (lInstances, channelsToMap, variables) {
    let self = this
    var i = 0
    ...
    Promise.all(
      Object.keys(lInstances).map(instanceID => {
        ...
        let bridge = self.loadInstance(instanceID, instanceData, channelsToMap, variables, i)
  ...
  async connectCCU () {
    ...
    if ((channelsToMap.length > 0) || (variablesToMap.length > 0)) {
      this.powerUpBridges(lInstances, channelsToMap, variablesToMap)
  ...
  loadInstance (instId, instanceData, ccuDevices, variables, portInc) {
    let self = this
