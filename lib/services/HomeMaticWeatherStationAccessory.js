...
class HomeMaticWeatherStationAccessory extends HomeMaticAccessory {
  publishServices (Service, Characteristic) {
    ...
    if (this.getDataPointNameFromSettings('SunShineDuration', null)) {
      ...
      this.registerAddressWithSettingsKeyForEventProcessingAtAccessory('SunShineDuration', null, (newValue) => {
        var duration = parseFloat(newValue)
        duration = (duration / 60).toFixed(2) // calculate hours
        self.currentSunshineCharacteristic.updateValue(duration, null)
      })
    }
  ...
  initServiceSettings () {
    return {
      ...
      'HmIP-SWO-B': {
        ...
        SunShineDuration: {name: 'SUNSHINEDURATION'},
        Windspeed: {name: 'WIND_SPEED'}
      },
      'HmIP-SWO-PR': {
        Temperature: {name: 'ACTUAL_TEMPERATURE'},
