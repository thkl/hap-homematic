...
class HomeMaticContactSensorAccessory extends HomeMaticAccessory {
    ...
    this.state = this.contact.getCharacteristic(Characteristic.ContactSensorState)
      .on('get', function (callback) {
        self.getValueForDataPointNameWithSettingsKey('state', true).then(value => {
          if (callback) {
            callback(null, self.getDataPointResultMapping('state', value))
          }
        })
      })

    this.registerAddressWithSettingsKeyForEventProcessingAtAccessory('state', (newValue) => {
      let mappedValue = self.getDataPointResultMapping('state', newValue)
  ...
  initServiceSettings () {
    return {
      '*': {
        state: {name: 'STATE', boolean: true, mapping: {true: 1, false: 0}}
      }
    }
  }
