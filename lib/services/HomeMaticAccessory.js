 ...
 */
  ...
  deviceServiceSettings (key, subkey) {
    // first try the channel Type
    this.log.debug('[Generic] get deviceServiceSettings for type %s subkey %s', key, subkey)
    let oDeviceSettings = this.serviceSettings[this._ccuType]
    // if not the device type
    ...
    this.log.debug('[Generic] deviceServiceSettings are %s', JSON.stringify(oDeviceSettings))

    if ((subkey === undefined) || (subkey === null)) {
      if (oDeviceSettings !== undefined) {
        this.log.debug('[Generic] return settings for key %s is %s', subkey, JSON.stringify(oDeviceSettings[key]))
      ...
      if (oDeviceSettings[subkey] !== undefined) {
        this.log.debug('[Generic] return settings for subkey %s', subkey)
  ...
  getDataPointResultMapping (type, subkey, value, mapTable = 'mapping', reverse = false) {
    this.log.debug('[Generic] Mapping - Input is %s', value)
    let settings = this.deviceServiceSettings(type, subkey)
    this.log.debug('[Generic] Mapping - Settings %s', JSON.stringify(settings))
    ...
    if ((typeof settings === 'object') && (mappingtable)) {
      ...
      if (settings.number) {
        ...
        this.log.debug('[Generic] Mapping - change to number')
  ...
  async addLowBatCharacteristic (rootService, channel = 0) {
    let self = this
    if (rootService !== undefined) {
      if (rootService.testCharacteristic(Characteristic.StatusLowBattery)) {
      ...
      if (channel !== undefined) {
        if (await this._ccu.hazDatapoint(this.buildAddress(channel + '.LOWBAT'))) {
          this.lowBatCharacteristic.on('get', async callback => {
            callback(null, self.isTrue(await self.getValue(channel + '.LOWBAT', true)))
          })
        ...
        if (await this._ccu.hazDatapoint(this.buildAddress(channel + '.LOW_BAT'))) {
          this.lowBatCharacteristic.on('get', async callback => {
            callback(null, self.isTrue(await self.getValue(channel + '.LOW_BAT', true)))
          })
  ...
  addBatteryLevelStatus (dpName = '0.OPERATING_VOLTAGE', dplowBat = '0.LOW_BAT', maxVoltage = 3.0) {
    let self = this
    ...
    this.registerAddressForEventProcessingAtAccessory(this.buildAddress(dplowBat), (newValue) => {
      self.lowLevelCharacteristic.updateValue(self.isTrue(newValue), null)
    })
