...
class HomeMaticAccessory extends EventEmitter {
  ...
  */
  ...
  isReadOnly () {
    ...
    return this.isChannelReadOnly(roChannel)
  }

  isChannelReadOnly (channelNum) {
    if (channelNum === undefined) {
      return (this.access) ? parseInt(this.access) !== 255 : true
    } else {
 ...
 */
  ...
  addBatteryLevelStatus (dpName = '0.OPERATING_VOLTAGE', dplowBat = '0.LOW_BAT', maxVoltage = 3.0) {
    ...
    this.registerAddressForEventProcessingAtAccessory(this.buildAddress(dplowBat), function (newValue) {
      self.lowLevelCharacteristic.updateValue(self.isTrue(newValue), null)
    })

    this.registerAddressForEventProcessingAtAccessory(this.buildAddress(dpName), function (newValue) {
  ...
  addFaultCharacteristic (rootService, hmDatapoint = '0.STICKY_UNREACH', mapping = (value) => { return self.isTrue(value) }) {
    ...
    this.registerAddressForEventProcessingAtAccessory(this.buildAddress(hmDatapoint), function (newValue) {
