...
class HomeMaticAccessory extends EventEmitter {
  constructor (channel, sInterface, server, settings = {}) {
    ...
    this.log.info('Checking defaultnane %s vs %s -> %s', cDefaultName, channel.name, idx)
    if (idx === -1) {
  ...
  */
  init () {
    ...
    this.serviceSettings = this.initServiceSettings()
 ...
 */
  ...
  getService (serviceType, name = this._name, forceAdd = false, subtype = '') {
    ...
    if ((!service) || (forceAdd === true)) {
      ...
      service = this.homeKitAccessory.addService(serviceType, serviceType.UUID, subtype)
    }
  ...
  getDataPointResultMapping (type, value) {
    let settings = this.deviceServiceSettings(type)
    let testValue = value
    if ((typeof settings === 'object') && (settings.mapping)) {
      if (settings.number) {
      ...
      if (settings.mapping) {
        ...
        if (settings.mapping[testValue] !== undefined) {
          this.log.debug('[Generic] mapping result found ...')
          return settings.mapping[testValue]
        } else {
          this.log.debug('[Generic] no value in mappingtable %s returning input', JSON.stringify(settings.mapping))
          return value
        }
      } else {
  ...
  getDataPointNameFromSettings (type) {
  ...
  setValueForDataPointNameWithSettingsKey (settingsKey, newValue) {
    let realDataPointName = this.getDataPointNameFromSettings(settingsKey)
  ...
  getValueForDataPointNameWithSettingsKey (settingsKey, ignoreCache) {
    let realDataPointName = this.getDataPointNameFromSettings(settingsKey)
  ...
  registerAddressWithSettingsKeyForEventProcessingAtAccessory (key, callback) {
    let adrForKey = this.getDataPointNameFromSettings(key)
  ...
  configurationItems () {
    this.log.warn('[GENERIC] OVERRIDE THIS TO SPECIFY THE CONFIG ITEMS FOR THE SERVICE')
    return []
  }
