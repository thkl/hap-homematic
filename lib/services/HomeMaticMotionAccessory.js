...
class HomeMaticMotionAccessory extends HomeMaticAccessory {
  publishServices (Service, Characteristic) {
    ...
    this.illuminationLevel = this.motionSensor.getCharacteristic(Characteristic.CurrentAmbientLightLevel)
      .on('get', function (callback) {
    ...
    this.registeraddressForEventProcessingAtAccessory(this.buildAddress(self.getDataPointName('illumination')), function (newValue) {
      self.illuminationLevel.updateValue(self.isTrue(newValue), null)
    })
  ...
  getDataPointName (type) {
    return {motion: 'MOTION', illumination: 'BRIGHTNESS'}[type]
  }
