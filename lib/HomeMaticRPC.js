...
class HomeMaticRPCClient {
  ...
  stop () {
    ...
    return new Promise((resolve, reject) => {
      ...
      self.client.methodCall('init', [self.rpcInit + self.localIP + ':' + self.listeningPort], (error, value) => {
...
class HomeMaticRPC extends EventEmitter {
    ...
    this.isPortTaken(this.listeningPort, function (error, inUse) {
      if (error === null) {
        if (inUse === false) {
          ...
          self.log.info('[RPC] server for all interfaces is listening on port %s.', self.listeningPort)
        } else {
          self.log.error('****************************************************************************************************************************')
  ...
  disconnectInterfaces () {
    ...
    this.stopped = true
    return Promise.all(self.interfaces.map(async function (iface) {
      await iface.stop()
    })
    )
  }
